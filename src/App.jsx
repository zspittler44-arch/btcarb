  ...
  const runAgent = useCallback(async (name) => {
    ...
    if (name === "rex") {
      ...
      if (latestLiqBias === "LONGS") {
        ...
        if (latestLiqSpike) { downScore += 1; downVotes.push("liq spike"); }
      } else if (latestLiqBias === "SHORTS") {
        // Short liquidation cascade → forced buying → bullish pressure
        upScore += 2; upVotes.push(`liq SHORTS${latestLiqSpike?" SPIKE":""}`);
        if (latestLiqSpike) { upScore += 1; upVotes.push("liq spike"); }
      }
      ...
      const aggSignal = aggDirection !== "NEUTRAL"
        ? `${aggDirection} — Score ${aggDirection==="UP"?upScore:downScore}/18. `
          ...
          + `funding=${fundingBullish?"↑":"↓"}, OI${avgOiDelta>=0?"+":""}${avgOiDelta.toFixed(2)}%, `
          + `liq=${latestLiqBias??"none"}${latestLiqSpike?" spike":""}, news=${latestNewsSentiment??"n/a"}, trend=${actualTrend}. `
        ...
        : `NEUTRAL — Insufficient conviction. ${debugStr}. `
          ...
          + `liq=${latestLiqBias??"none"}, news=${latestNewsSentiment??"n/a"}`;
